...
void BGRImgAnalyzer::ProcessImg(const cv::Mat &img){
    ...
    img_p = &imgClone;
    _img = img_p;
    _table.GenerateColorTable(_gridSize);
...
int BGRImgAnalyzer::GetPixelId(cv::Point pixelCoordinates, int maxColums){
    int row = pixelCoordinates.x;
    int col = pixelCoordinates.y;
    return row*maxColums + col;
}

cv::Point BGRImgAnalyzer::GetPixelCoordinates(int pixelId, int maxColums){
